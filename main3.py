...
class Library:
    ...
    def get_lib_score(self):
        return sum((book.score for book in self.books)) / self.ship
    ...
    def scan(self, already_scanned=None):

        if already_scanned:
            self.books_to_scan = [b for b in self.books_to_scan if b not in already_scanned]
